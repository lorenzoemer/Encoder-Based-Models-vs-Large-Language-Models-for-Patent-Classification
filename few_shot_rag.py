#!/usr/bin/env python3
"""
Few-shot + allowed-set retrieval evaluation of LLMs for CPC multi-label patent classification.

Inputs:
- TRAIN TSV: columns title, abstract, labels
- TEST TSV: columns title, abstract, labels
- CPC definitions JSON: {"A01B": "...", "G06F": "...", ...}
- Local embedding model directory (E5-base-v2)
- Local LLM directories

Outputs per model under: <output_dir>/<model_name>/
- metrics.json
- predictions.jsonl (includes prompt and retrieval metadata: static_ids, dynamic_ids, allowed_topk)
...
def parse_predicted_labels(text: str, allowed_set: set, max_labels: int) -> List[str]:
    if not text:
        return []

    def postprocess(labs: List[str]) -> List[str]:
        norm = [normalize_cpc_label(x) for x in labs]
        norm = [x for x in norm if x and x in allowed_set]
...
def enforce_nonempty(pred: List[str], raw_text: str, allowed_list: List[str], max_labels: int) -> List[str]:
    ...
    if allowed_list:
        top1 = normalize_cpc_label(allowed_list[0])
        if top1:
            return [top1]
...
def prepare_tfidf_retriever(train_texts: List[str], max_features: int, ngram_range: Tuple[int, int], min_df: int):
...
def build_fewshot_block_for_one(
    query_text: str,
    ...
    blocks: List[str] = []
    meta: Dict[str, Any] = {"static_ids": [], "dynamic_ids": []}

    use_static = static_ids[:static_k]
    ex_idx = 1
    for sid in use_static:
        blocks.append(format_fewshot_example(train_texts[sid], train_labels[sid], idx=ex_idx, max_labels=max_labels))
        meta["static_ids"].append(int(sid))
        ex_idx += 1

    if dynamic_k > 0:
        q = vectorizer.transform([query_text])
        sims = linear_kernel(q, X_train).ravel()

        for sid in use_static:
            if 0 <= sid < len(sims):
                sims[sid] = -1.0

        kth = min(dynamic_k, max(1, len(sims) - 1))
        topk = np.argpartition(-sims, kth=kth)[:dynamic_k]
        topk = topk[np.argsort(-sims[topk])]

        for did in topk:
            did = int(did)
            blocks.append(format_fewshot_example(train_texts[did], train_labels[did], idx=ex_idx, max_labels=max_labels))
            meta["dynamic_ids"].append(did)
            ex_idx += 1
...
def generate_batch(
    ...
    for title, abstract, allowed, fs_block in zip(titles, abstracts, allowed_lists, fewshot_blocks):
...
def main() -> None:
    ...
    train_texts = [build_text(ex.get(args.title_col, "") or "", ex.get(args.abstract_col, "") or "") for ex in train_ds]
    train_labels: List[List[str]] = []
    for ex in train_ds:
    ...
    vectorizer, X_train = prepare_tfidf_retriever(
    ...
    for model_name, model_path_str in args.model:
        ...
        allowed_lists: List[List[str]] = []
        for i in tqdm(range(0, len(test_texts), args.batch_size), desc=f"Allowed-set [{model_name}]"):
            btxt = test_texts[i : i + args.batch_size]
        ...
        fewshot_blocks: List[str] = []
        fewshot_meta: List[Dict[str, Any]] = []

        for qtext in test_texts:
            fs_block, meta = build_fewshot_block_for_one(
                query_text=qtext,
                train_texts=train_texts,
            ...
            fewshot_blocks.append(fs_block)
            fewshot_meta.append(meta)
        ...
        if args.use_codecarbon and EmissionsTracker is not None:
            tracker = EmissionsTracker(
                project_name=f"few_shot_rag_{model_name}_topk{args.top_k}",
        ...
        for i in tqdm(range(0, len(test_titles), args.batch_size), desc=f"Inference [{model_name}]"):
            ...
            balw = allowed_lists[i : i + args.batch_size]
            bfs = fewshot_blocks[i : i + args.batch_size]

            batch_preds, batch_raw, batch_prompts, batch_retr_meta = generate_batch(
                model=model,
                tokenizer=tok,
                titles=bt,
                abstracts=ba,
            ...
            for j in range(len(batch_retr_meta)):
                meta = {}
                meta.update(fewshot_meta[i + j])
                meta.update(batch_retr_meta[j])
                retrieval_meta_all.append(meta)
        ...
        metrics = {
            ...
            "rag": {"retriever": "e5-base-v2", "top_k": int(args.top_k), "max_labels_per_pred": int(args.max_labels)},
        }
        (outdir / "metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")

        with (outdir / "predictions.jsonl").open("w", encoding="utf-8") as f:
            for idx, (t, a, ptxt, gold, pred, rawtxt, prompt, meta, alw) in enumerate(
                zip(
                    test_titles,
                    test_abstracts,
                    test_texts,
                ...
                rec = {
                    ...
                    "prompt": prompt,
                    "retrieval": {**meta, "allowed_topk": [normalize_cpc_label(x) for x in alw]},
                }
